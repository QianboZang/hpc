...
int main(int argc, char *argv[])
{
    ...
    //  to the top
    MPI_Irecv(
        &data[1 + DOMAINSIZE * (DOMAINSIZE - 1)], SUBDOMAIN, MPI_DOUBLE, 
        rank_bottom, rank, comm_cart, &request
    );
    MPI_Send(
        &data[1 + DOMAINSIZE], SUBDOMAIN, MPI_DOUBLE, 
        rank_top, rank_top, comm_cart
    );
    ...
    //  to the bottom
    MPI_Irecv(
        &data[1], SUBDOMAIN, MPI_DOUBLE, 
        rank_top, rank, comm_cart, &request
    );
    MPI_Send(
        &data[1 + SUBDOMAIN * DOMAINSIZE], SUBDOMAIN, MPI_DOUBLE, 
        rank_bottom, rank_bottom, comm_cart
    );
    MPI_Wait(&request, &status);
    ...
    //  to the left
    MPI_Irecv(
        &data[-1 + 2 * DOMAINSIZE], 1, data_ghost, 
        rank_right, rank, comm_cart, &request
    );
    MPI_Send(
        &data[1 + DOMAINSIZE], 1, data_ghost, 
        rank_left, rank_left, comm_cart
    );
    MPI_Wait(&request, &status);
    ...
    //  to the right
    MPI_Irecv(
        &data[DOMAINSIZE], 1, data_ghost, 
        rank_left, rank, comm_cart, &request
    );
    MPI_Send(
        &data[-2 + 2 * DOMAINSIZE], 1, data_ghost, 
        rank_right, rank_right, comm_cart
    );
    MPI_Wait(&request, &status);
