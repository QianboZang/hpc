...
int main() {
    ...
    cout << "Serial execution time = " << time_serial << " sec, ";

    long double alpha_parallel = 0;
    double time_red_start, time_red = 0;
    double time_critical_start = 0, time_critical = 0;

    //   TODO: Write parallel version (2 ways!)
    //   i.  Using reduction pragma
    time_red_start = wall_time();
    for (int iterations = 0; iterations < NUM_ITERATIONS; ++iterations) {
        alpha_parallel = 0.0;
    ...
    time_red = wall_time() - time_red_start;

    //   ii. Using  critical pragma
    time_critical_start = wall_time();
    for (int iterations = 0; iterations < NUM_ITERATIONS; ++iterations) {
        alpha_parallel = 0.0;
        #pragma omp parallel
        {
