...
class Field:
    ...
    def exchange_startall(self):
        ...
        if domain.neighbour_north >= 0:
            self._buffN[:] = self.inner[:,-1]
            self._reqs.append(
                domain.comm.Isend([self._buffN, MPI.DOUBLE], domain.neighbour_north, tag=domain.rank)
            )
            self._reqs.append(
                domain.comm.Irecv([self._bdryN, MPI.DOUBLE], 
                domain.neighbour_north, tag=domain.neighbour_north)
            )
        if domain.neighbour_south >= 0:
            self._buffS[:] = self.inner[:,0]
            self._reqs.append(
                domain.comm.Isend([self._buffS, MPI.DOUBLE], domain.neighbour_south, tag=domain.rank)
            )
            self._reqs.append(
                domain.comm.Irecv([self._bdryS, MPI.DOUBLE], 
                domain.neighbour_south, tag=domain.neighbour_south)
            )
        if domain.neighbour_west >= 0:
            self._buffW[:] = self.inner[0,:]
            self._reqs.append(
                domain.comm.Isend([self._buffW, MPI.DOUBLE], 
                domain.neighbour_west, tag=domain.rank)
            )
            self._reqs.append(
                domain.comm.Irecv([self._bdryW, MPI.DOUBLE], 
                domain.neighbour_west, tag=domain.neighbour_west)
            )
        if domain.neighbour_east >= 0:
            self._buffE[:] = self.inner[-1,:]
            self._reqs.append(
                domain.comm.Isend([self._buffE, MPI.DOUBLE], 
                domain.neighbour_east, tag=domain.rank)
            )
            self._reqs.append(
                domain.comm.Irecv([self._bdryE, MPI.DOUBLE],
                domain.neighbour_east, tag=domain.neighbour_east)
            )
    ...
    def exchange_waitall(self):
        ...
        # ... implement ...
        MPI.Request.Waitall(self._reqs)
